...
function checkPrereqs() {
  ...
  DOCKER_IMAGE_VERSION=$(${CONTAINER_CLI} run --rm hyperledger/fabric-peer:3.1.5 peer version | sed -ne 's/^ Version: //p')
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    CA_DOCKER_IMAGE_VERSION=$(${CONTAINER_CLI} run --rm hyperledger/fabric-ca:1.5.22 fabric-ca-client version | sed -ne 's/ Version: //p' | head -1)
...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cfssl" ]; then
    ...
    peer_cert peer peer0.centralbank.sworna.example.com org1
    peer_cert admin Admin@centralbank.sworna.example.com org1

    infoln "Creating Org2 Identities"
    #function_name cert-type   CN   org
    peer_cert peer peer0.banka.sworna.example.com org2
    peer_cert admin Admin@banka.sworna.example.com org2

    infoln "Creating Orderer Org Identities"
    #function_name cert-type   CN   
    orderer_cert orderer orderer.sworna.example.com
    orderer_cert admin Admin@sworna.example.com
