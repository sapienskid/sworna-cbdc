...
if jq -e --arg MSP "$MSP" '.channel_group.groups.Application.groups | has($MSP)' \
  ...
  # Sign locally for any co-located bank admins (if present)
  ...
  for bank_msp in $existing_banks; do
    bank_num=$(echo "$bank_msp" | grep -oP '\d+' || true)
    [ -n "$bank_num" ] || continue
    ...
    if [ -d "$bank_admin_msp" ]; then
      ...
      peer channel signconfigtx -f "$ARTIFACTS/update_envelope.pb" || true
    fi
  done

  infoln "submitting the config update to add ${MSP} with CentralBankMSP authority"
  setGlobals 1
